...
exports.create = async (req, res) => {
    try {
        ...
        res.redirect('/staff?success=Staff record successfully added!');
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error: ' + err.message);
    }
...
exports.edit = async (req, res) => {
    try {
        ...
        if (!staff) return res.redirect('/staff?error=Staff Not Found');
...
exports.update = async (req, res) => {
    try {
        ...
        res.redirect('/staff?success=Staff record successfully updated!');
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error: ' + err.message);
    }
...
exports.delete = async (req, res) => {
    try {
        ...
        res.redirect('/staff?success=Staff record permanently deleted.');
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error: ' + err.message);
    }
