...
exports.dashboard = async (req, res) => {
    try {
        ...
        const [reportedRows] = await db.query(`
            SELECT
                COUNT(*) AS totalReportedChildren,
                SUM(CASE WHEN status = 'Pending' THEN 1 ELSE 0 END) AS pendingReports
            FROM reported_children
        `).catch(() => [[{ totalReportedChildren: 0, pendingReports: 0 }]]);
        res.render('children/dashboard', {
            stats,
            recentChildren,
            adoptionStats,
            reportedStats: reportedRows[0] || { totalReportedChildren: 0, pendingReports: 0 },
...
exports.create = async (req, res) => {
    try {
        ...
        if (existing) {
            ...
            return res.redirect(`/children?success=${encodeURIComponent('Child registered successfully.')}`);
        }
