...
function fileFilter(req, file, cb) {
    ...
    if (file.fieldname === 'child_photo' || file.fieldname === 'item_photo' || file.fieldname === 'item_image') {
        if (imageMime.has(file.mimetype)) return cb(null, true);
...
module.exports = {
    ...
    uploadDonationItemPhoto: (req, res, next) => {
        donationUpload(req, res, (err) => {
            if (err) req.fileUploadError = err.message || 'File upload failed.';
            if (!req.file && req.files) {
                req.file = (req.files.item_photo && req.files.item_photo[0]) || (req.files.item_image && req.files.item_image[0]) || null;
            }
            next();
        });
    },
