...
exports.getAdoptionById = async (req, res) => {
    try {
        ...
        if (!adoption) req.flash('error', 'Record not found');
...
exports.updateAdoption = async (req, res) => {
    try {
        ...
        if (!existing) req.flash('error', 'Record not found');
        return res.redirect('/adoptions');
        ...
        req.flash('success', 'Adoption record updated.');
        res.redirect(`/adoptions/${req.params.id}`);
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error: ' + err.message);
    }
...
exports.acceptAdoption = async (req, res) => {
    try {
        ...
        if (existing.adoption_status !== 'Pending') {
            req.flash('error', 'Only pending applications can be accepted.');
            req.flash('success', String('Application accepted. You can now schedule the orphanage visit.'));
            return res.redirect(`/adoptions/${req.params.id}`);
        }
...
exports.scheduleAdoptionVisit = async (req, res) => {
    try {
        ...
        if (!['Accepted', 'Scheduled', 'Approved'].includes(existing.adoption_status)) {
            req.flash('error', 'Only accepted applications can be scheduled.');
            return res.redirect(`/adoptions/${req.params.id}`);
        }
        const rawWhen = String(req.body.visit_scheduled_at || '').trim();
        if (!rawWhen) {
            req.flash('error', 'Visit date and time is required.');
            return res.redirect(`/adoptions/${req.params.id}`);
        }
        const parsed = new Date(rawWhen);
        if (Number.isNaN(parsed.getTime())) {
            req.flash('error', 'Invalid visit date and time.');
            return res.redirect(`/adoptions/${req.params.id}`);
        }
        ...
        if (!scheduled) {
            req.flash('error', 'Could not save schedule. Please refresh and try again.');
            return res.redirect(`/adoptions/${req.params.id}`);
        }
        ...
        try {
            ...
            if (emailResult && emailResult.skipped) {
                req.flash('success', 'Visit/interview scheduled successfully. Email notification was skipped because SMTP is not fully configured.');
            return res.redirect(`/adoptions/${req.params.id}`);
            }
            req.flash('success', 'Visit/interview scheduled and email notification sent.');
            return res.redirect(`/adoptions/${req.params.id}`);
        } catch (mailErr) {
...
exports.deleteAdoption = async (req, res) => {
    try {
        const existing = await Adoption.findById(req.params.id);
        if (!existing) req.flash('error', 'Record not found');
        return req.flash('success', 'Adoption record deleted.');
